...
simulate_diffusion <- function(net = NULL, sim_runs = 10, p_copy_high = .5, p_copy_low = .5, age_diff = FALSE, treatments = treatments) {
  ...
  network <- graph.data.frame(d = net, directed = "TRUE")  #directed
  
  ## remove participants who did not fill nominations (number startin with 88)
  ...
  if (components(network)$no < 2) {
    ...
    #plot directed network while runing the sim
    # plot(
    #   network,
    ...
    #   edge.color = "black",
    #   edge.arrow.size  = .1,
    #   vertex.size = V(network)$indeg*1.2,
    #   vertex.label = V(network)$indeg,
    #   main = paste("Network n", n)
    # )
    ...
    for (t in 1:sim_runs){
      ...
      if (t<=sim_runs/2) {
        ...
        ## give B to one of top 3 status individuals @Andrea: should this be just top 1?
        ...
        to_sample <- c(
          #length(low_status_nodes) - 4,
          length(low_status_nodes) - 2,
          length(low_status_nodes) - 1
        )
        
        # give B to one of bottom 3 low status individuals
